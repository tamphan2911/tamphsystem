          ...
          <div key={q.id}>
            ...
            <div className="space-y-3">
              {q.options.map((opt) => {
                ...
                let cardClass =
                  "border-slate-200 dark:border-slate-700 hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/10 cursor-pointer";
                let icon: ReactNode = null;

                if (isSelected) {
                  cardClass =
                    "border-blue-500 bg-blue-50 dark:bg-blue-900/20 ring-1 ring-blue-500 cursor-pointer";
                }

                if (isSubmitted) {
                  cardClass =
                    "border-slate-200 dark:border-slate-800 opacity-50 cursor-default"; // Default disabled

                  if (isSelected && !isCorrect) {
                    cardClass =
                      "border-red-500 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 cursor-default";
                    icon = <XCircle className="w-5 h-5 text-red-500" />;
                  } else if (isCorrect) {
                    cardClass =
                      "border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 font-medium cursor-default ring-1 ring-emerald-500";
                    icon = (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-slate-700 dark:text-slate-300 ${isSubmitted && (isCorrect || (isSelected && !isCorrect)) ? "font-medium" : ""}`}
                      >
                        {opt.text}
                      </span>
                    </div>
      ...
      <div className="mt-10 pt-6 border-t border-slate-200 dark:border-slate-800 flex justify-end">
        {!isSubmitted ? (
          <button
            ...
            disabled={
              Object.keys(answers).length !== questions.length || isPending
            }
