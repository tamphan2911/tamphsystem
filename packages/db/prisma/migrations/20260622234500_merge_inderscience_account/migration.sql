DROP TABLE IF EXISTS "inderscience_account_target";
CREATE TEMP TABLE "inderscience_account_target" AS
...
WHERE publisher."normalizedName" = 'inderscience'
...
DROP TABLE IF EXISTS "inderscience_account_merge_map";
CREATE TEMP TABLE "inderscience_account_merge_map" AS
SELECT
  account."id" AS "duplicateId",
  target."survivorId"
FROM "PublisherAccount" AS account
JOIN "inderscience_account_target" AS target
  ON target."publisherId" = account."publisherId"
WHERE LOWER(BTRIM(account."username")) = 'tamphan2911'
  AND account."id" <> target."survivorId";

UPDATE "ResearchSubmission" AS submission
SET "accountId" = merge_map."survivorId"
FROM "inderscience_account_merge_map" AS merge_map
...
SET
  "username" = 'tamphan2911',
  "password" = '291189TamPhan',
...
FROM "inderscience_account_target" AS target
LEFT JOIN (
  SELECT
...
USING "inderscience_account_merge_map" AS merge_map
...
SET
  ...
  "updatedAt" = CURRENT_TIMESTAMP
FROM "inderscience_account_target" AS target
WHERE publisher."id" = target."publisherId";
